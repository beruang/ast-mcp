...
#[serde(default)]
pub struct AstFindImportsInput {
    pub file_path: String,
}

pub fn handle(workspace: &Workspace, args: serde_json::Value) -> serde_json::Value {
    let input: AstFindImportsInput = match serde_json::from_value(args) {
        Ok(v) => v,
    ...
    let imports = imports::find_imports(&tree, &source, lang);
