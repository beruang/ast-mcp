...
#[derive(Deserialize)]
#[serde(default)]
pub struct AstFindExportsInput {
    pub file_path: String,
    pub include_best_effort_python: bool,
}
...
pub fn handle(workspace: &Workspace, args: serde_json::Value) -> serde_json::Value {
    let input: AstFindExportsInput = match serde_json::from_value(args) {
        Ok(v) => v,
    ...
    let exports = exports::find_exports(&tree, &source, lang, input.include_best_effort_python);
