...
/// Return outline-significant nodes from the root of a JavaScript/JSX file.
///
/// Matches: import_statement, export_statement, function_declaration,
/// generator_function_declaration, class_declaration, and lexical_declaration
/// (const / let / var, including `require()` calls).
///
/// `export_statement` is unwrapped so that exported declarations are
/// reported by their inner kind (e.g. `class_declaration`).
///
/// Class bodies are recursed into for method_definition children.
pub fn outline_candidates(root: tree_sitter::Node, source: &str) -> Vec<OutlineCandidate> {
    ...
    for child in root.children(&mut cursor) {
        ...
        match kind {
            ...
            "lexical_declaration" => {
                // Include const / let / var (also covers require() patterns).
                candidates.push(outline::make_candidate(&child, source));
            }
            _ => {}
        }
        ...
        if let Some(class_def) = class_node {
            if class_def.kind() == "class_declaration" {
                ...
                if let Some(idx) = candidates.iter().position(|c| {
                    ...
                    collect_class_members_js(&class_def, source, class_candidate);
                }
            }
        }
    }
...
/// Walk a class body and add method_definition children to the candidate.
fn collect_class_members_js(
