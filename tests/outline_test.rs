...
fn outline_typescript_classes_returns_kinds_and_names() {
    let ws = workspace();
    let result =
        tools::file_outline::handle(&ws, json!({"file_path": "tests/fixtures/outline/classes.ts"}));
    ...
    let text = result["outlineText"].as_str().expect("outlineText should be a string");
...
fn outline_python_classes_returns_methods() {
    let ws = workspace();
    let result =
        tools::file_outline::handle(&ws, json!({"file_path": "tests/fixtures/outline/classes.py"}));
    assert!(result["error"].is_null(), "unexpected error: {:?}", result);
...
fn outline_typescript_types_includes_interface_and_type_and_enum() {
    ...
    let kinds: Vec<&str> = nodes.iter().map(|n| n["kind"].as_str().unwrap_or("")).collect();
    ...
    assert!(kinds.contains(&"enum_declaration"), "should have enum_declaration, got: {:?}", kinds);
}
...
fn outline_with_max_depth() {
    ...
    if let Some(class_node) = nodes.iter().find(|n| n["kind"] == "class_declaration") {
        ...
        if let Some(kids) = children {
            assert!(kids.is_empty(), "at depth 0, children should be empty or null");
        }
    }
}
...
fn outline_without_ranges() {
    ...
    if let Some(first) = nodes.first() {
        assert!(first["range"].is_null(), "range should be null when include_ranges is false");
    }
}
