...
fn find_imports_typescript_all_forms() {
    ...
    assert!(imports.len() >= 6, "expected at least 6 imports, got {}", imports.len());

    // Check for at least one of each kind
    let kinds: Vec<&str> = imports.iter().map(|i| i["kind"].as_str().unwrap()).collect();
    assert!(kinds.contains(&"import"), "expected at least one 'import' kind, got: {:?}", kinds);
    assert!(kinds.contains(&"require"), "expected at least one 'require' kind");
    assert!(kinds.contains(&"dynamic_import"), "expected at least one 'dynamic_import' kind");
}
...
fn find_imports_typescript_default() {
    ...
    let imports = result["imports"].as_array().unwrap();
    let react_import =
        imports.iter().find(|i| i["modulePath"].as_str() == Some("react") && i["kind"] == "import");
    assert!(react_import.is_some(), "expected 'import React from \"react\"'");
    ...
    let has_react = names.iter().any(|n| n["imported"].as_str() == Some("React"));
...
fn find_imports_typescript_side_effect() {
    ...
    let imports = result["imports"].as_array().unwrap();
    let side_effect = imports.iter().find(|i| i["modulePath"].as_str() == Some("reflect-metadata"));
    assert!(side_effect.is_some(), "expected side-effect import 'reflect-metadata'");
}
...
fn find_imports_python_all_forms() {
    ...
    let kinds: Vec<&str> = imports.iter().map(|i| i["kind"].as_str().unwrap()).collect();
    assert!(kinds.contains(&"import"), "expected 'import' kind: {:?}", kinds);
    assert!(kinds.contains(&"from_import"), "expected 'from_import' kind: {:?}", kinds);

    // Check for wildcard import
    let wildcard = imports.iter().find(|i| {
        i["names"].as_array().is_some_and(|n| n.iter().any(|a| a["imported"].as_str() == Some("*")))
    });
...
fn find_exports_typescript_all_forms() {
    ...
    assert!(exports.len() >= 8, "expected at least 8 exports, got {}", exports.len());

    let kinds: Vec<&str> = exports.iter().map(|e| e["kind"].as_str().unwrap()).collect();
...
fn find_exports_typescript_class_name() {
    ...
    let exports = result["exports"].as_array().unwrap();
    let calc = exports.iter().find(|e| e["name"].as_str() == Some("Calculator"));
...
fn find_exports_typescript_default() {
    ...
    let exports = result["exports"].as_array().unwrap();
    let default_exports: Vec<_> = exports.iter().filter(|e| e["isDefault"] == true).collect();
...
fn find_exports_typescript_re_export() {
    ...
    let exports = result["exports"].as_array().unwrap();
    let from_greetings =
        exports.iter().find(|e| e["reExportSource"].as_str() == Some("./greetings"));
    assert!(from_greetings.is_some(), "expected re-export from './greetings'");
}
...
fn find_exports_python_public_defs() {
    let ws = workspace();
    let result =
        tools::find_exports::handle(&ws, json!({"file_path": "tests/fixtures/exports/public.py"}));
    assert!(result["error"].is_null(), "unexpected error: {:?}", result);
    let exports = result["exports"].as_array().unwrap();
    assert!(exports.len() >= 2, "expected at least 2 Python public exports, got {}", exports.len());

    // Should have function 'add'
    let add = exports.iter().find(|e| e["name"].as_str() == Some("add") && e["kind"] == "function");
    assert!(add.is_some(), "expected public function 'add'");

    // Should have class 'Calculator'
    let calc =
        exports.iter().find(|e| e["name"].as_str() == Some("Calculator") && e["kind"] == "class");
...
fn find_exports_python_excludes_private() {
    ...
    let result =
        tools::find_exports::handle(&ws, json!({"file_path": "tests/fixtures/exports/public.py"}));
    let exports = result["exports"].as_array().unwrap();
    let private_func = exports.iter().find(|e| e["name"].as_str() == Some("_private_helper"));
    assert!(private_func.is_none(), "private function should be excluded");

    let private_class = exports.iter().find(|e| e["name"].as_str() == Some("_PrivateClass"));
